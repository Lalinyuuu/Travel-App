    ...
    <main class="py-8">
      <div class="container">
        <section class="text-center mb-12 py-8">
          <h2
            class="text-3xl md:text-4xl font-bold text-secondary mb-6 transition-colors duration-300"
          >
            {{ $t("landing.searchTitle") }}
          </h2>
          <div
            class="flex flex-col md:flex-row items-center gap-4 max-w-2xl mx-auto"
          >
            ...
            <button
              @click="handleSearch"
              class="btn btn-primary whitespace-nowrap px-6 py-3 rounded-xl font-semibold"
            >
              {{ $t("landing.search") }}
            </button>
          </div>
        ...
        <section id="trips-results">
          <div
            v-if="loading"
            class="text-center py-16 text-(--color-text-secondary)"
          >
          ...
          <div
            ...
            class="flex justify-center items-center min-h-[400px] py-16"
          >
            <div class="text-center text-(--color-text-secondary)">
              ...
              <h3
                v-if="displaySearchQuery"
                class="text-2xl mb-2 text-(--color-text)"
              >
                {{ $t("landing.noTripsFound") }}
              </h3>
              <p v-if="displaySearchQuery" class="text-base">
                {{ $t("landing.noTripsFoundDesc") }}
              </p>
          ...
          >
            ...
            >
              <div
                class="w-full h-64 min-h-64 max-h-64 overflow-hidden bg-(--color-background-light) relative shrink-0 rounded-t-xl"
              >
                <img
                  v-if="trip.coverImage"
                  :src="trip.coverImage"
                  :alt="getTranslatedTitle(trip)"
                ...
                <div
                  v-else
                  class="w-full h-full flex items-center justify-center text-gray-4"
                >
                  <svg
                    width="60"
                    height="60"
              ...
              <div
                class="p-6 flex-1 flex flex-col min-w-0 bg-(--color-card-bg) shrink-0"
              >
                <div class="flex justify-between items-start mb-3 gap-6">
                  <h3
                    class="text-xl font-semibold text-(--color-text) m-0 flex-1 line-clamp-2 leading-tight"
                  >
                    {{ getTranslatedTitle(trip) }}
                  </h3>
                  <span
                    v-if="trip.province"
                    class="text-sm text-white bg-province-tag px-3 py-1 rounded-xl whitespace-nowrap shrink-0 font-medium transition-colors duration-300"
                    >{{ translateTag(trip.province) }}</span
                  >
                </div>
                <p
                  v-if="
                    ...
                    trip.created_at
                  "
                  class="text-xs my-2 transition-colors duration-300"
                ...
                <p
                  class="text-sm text-(--color-text-secondary) mb-4 line-clamp-3 flex-1 min-h-18 leading-relaxed"
                >
                  {{ getTranslatedShortDescription(trip) }}
                </p>
              ...
              <button
                class="absolute right-4 top-4 bg-white/90 dark:bg-[rgba(36,73,107,0.9)] border-none cursor-pointer p-2 text-primary flex items-center justify-center rounded-full transition-all duration-300 z-10 shadow-md dark:shadow-[0_4px_6px_-1px_rgba(0,0,0,0.3)] hover:bg-[#FFC0BE] dark:hover:bg-secondary hover:scale-110 hover:shadow-[0_10px_15px_-3px_rgba(255,130,169,0.2)] dark:hover:shadow-[0_10px_15px_-3px_rgba(68,109,146,0.3)]"
                @click.stop="handleShare(trip)"
                :title="$t('landing.shareLink')"
              >
                <svg
                  width="20"
                  height="20"
          ...
          <div v-if="!loading && trips.length > 0" class="text-center py-8">
            ...
            <div
              v-if="!hasMore && trips.length > 0"
              class="text-(--color-text-secondary) text-sm mt-4"
            >
              <p>
                {{
                  $t("landing.showing", {
...
const { getTranslatedTitle, getTranslatedShortDescription } =
  useTripTranslation();
...
// Auto-scroll to results when search query present
watch(
  () => route.query.search,
  async (newSearch) => {
    if (newSearch) {
      await nextTick();
      setTimeout(() => {
        const element = document.getElementById("trips-results");
        if (element) {
